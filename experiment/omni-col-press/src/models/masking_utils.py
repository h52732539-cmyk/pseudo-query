...
def full_attention_mask_function(batch_idx: int, head_idx: int, q_idx: int, kv_idx: int) -> bool:
    """Allows all query/key pairs to attend to each other (bidirectional attention)."""
...
def sliding_window_overlay(sliding_window: int) -> Callable:
    """
    This is an overlay depicting a symmetric sliding window pattern around each query token.
    """

    def inner_mask(batch_idx: int, head_idx: int, q_idx: int, kv_idx: int) -> bool:
        return abs(kv_idx - q_idx) < sliding_window
...
def chunked_overlay(chunk_size: int, left_padding: torch.Tensor) -> Callable:
    """
    This is an overlay depicting a chunked attention pattern.
    """
...
def sliding_window_causal_mask_function(sliding_window: int) -> Callable:
    ...
    """
    return and_masks(sliding_window_overlay(sliding_window), full_attention_mask_function)


def chunked_causal_mask_function(chunk_size: int, left_padding: torch.Tensor) -> Callable:
    """
    This return the mask_function function to create a bidirectional chunked attention mask.
    """
    if not _is_torch_greater_or_equal_than_2_6:
        return and_masks(_legacy_chunked_overlay(chunk_size), full_attention_mask_function)
    return and_masks(chunked_overlay(chunk_size, left_padding), full_attention_mask_function)
...
def create_causal_mask(
    ...
    """
    Create a standard bidirectional attention mask based on the attention implementation used (stored in the
    config). If `past_key_values` has an hybrid cache structure, this function will return the mask corresponding
    ...
    mask_factory_function = full_attention_mask_function
...
def create_sliding_window_causal_mask(
    ...
    """
    Create a symmetric sliding window attention mask based on the attention implementation used (stored in the
    config). This type of attention pattern was mostly democratized by Mistral. If `past_key_values` has an
    hybrid cache structure, this function will return the mask corresponding to one of the "sliding_attention"
    layers (to align to what is needed in the `modeling_xxx.py` files).
...
def create_chunked_causal_mask(
    ...
    """
    Create a chunked bidirectional attention mask based on the attention implementation used (stored in the config).
    This type of attention pattern was mostly democratized by Llama4. If `past_key_values` has an hybrid cache
    structure, this function will return the mask corresponding to one of the "chunked_attention" layers (to align to
    what is needed in the `modeling_xxx.py` files).
