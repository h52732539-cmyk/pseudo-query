...
from src.models.masking_utils import create_causal_mask, create_sliding_window_causal_mask
...
def eager_attention_forward(
    ...
    **kwargs,
    ...
    if attention_mask is not None:
        # Handle both 2D and 4D attention masks
        if attention_mask.ndim == 2:
            # Convert 2D mask [batch_size, seq_len] to 4D format
            # This should ideally be handled upstream, but we handle it here for robustness
            batch_size, seq_len = attention_mask.shape
            query_len = query.shape[2]
            key_len = key_states.shape[-2]
            
            # Expand 2D mask to 4D: [batch_size, 1, query_len, key_len]
            # attention_mask: 1 = valid token, 0 = padding token
            # We need: 0 = valid, -inf = masked
            min_val = torch.finfo(query.dtype).min
            
            # Create 4D mask
            if key_len <= seq_len:
                mask_2d = attention_mask[:, :key_len]  # [batch_size, key_len]
            else:
                # Pad if key_len > seq_len (shouldn't happen normally)
                padding = torch.ones(batch_size, key_len - seq_len, 
                                    device=attention_mask.device, dtype=attention_mask.dtype)
                mask_2d = torch.cat([attention_mask, padding], dim=-1)
            
            # Expand to 4D: [batch_size, 1, query_len, key_len]
            mask_4d = mask_2d.unsqueeze(1).unsqueeze(2).expand(batch_size, 1, query_len, key_len)
            
            # Convert: 1 -> 0 (valid), 0 -> -inf (masked)
            causal_mask = torch.where(
                mask_4d.bool(),
                torch.tensor(0.0, device=mask_4d.device, dtype=query.dtype),
                torch.tensor(min_val, device=mask_4d.device, dtype=query.dtype)
            )
        elif attention_mask.ndim == 4:
            # Handle the sdpa style attention mask
            ...
            if attn_implementation == "sdpa":
                ...
                causal_mask = torch.where(attention_mask, torch.tensor(0.0, device=attention_mask.device, dtype=query.dtype), min_dtype)
            # Already 4D, slice if needed
            causal_mask = attention_mask[:, :, :, : key_states.shape[-2]]
        else:
            raise ValueError(f"attention_mask must be 2D or 4D, got {attention_mask.ndim}D")
...
class Qwen2_5_VLRotaryEmbedding(nn.Module):
    ...
    def __init__(self, config: Qwen2_5_VLTextConfig, device=None):
        ...
        self.config = config
        # transformers >= 4.52: 'default' rope type is no longer in ROPE_INIT_FUNCTIONS,
        # it must be computed inline (plain 1/theta^(2i/d) formula).
        if self.rope_type == "default":
            self.rope_init_fn = None
            # transformers >= 5.x: rope_theta lives in config.rope_parameters dict
    ...
    def compute_default_rope_parameters(self, config=None):
        """Called by transformers >= 4.52 _init_weights for 'default' rope type."""
        ...
        return inv_freq, self.attention_scaling
